...
class TreeAVL(object):
    ...
    def add(self, payload=None, root=None):
        ...
        for n in reversed(search_queue):
            n._height = max(n._leftChild._height if n._leftChild else 0,
                            n._rightChild._height if n._rightChild else 0) + 1
    ...
    def remove(self, value, start=None):
        ...
        if value < start._payload:
            ...
            while x._parent != None:
                x = x._parent
                x._height = max(
                    self.height(x._leftChild), self.height(x._rightChild)) + 1
    ...
    def balance(self, node, balance):
        if balance < -1:
            if node._rightChild != None:
                # print('Balancing -1')
                if node._rightChild._payload < node._payload:
            ...
            if node._leftChild != None:
                # print('Balancing 1')
                if node._leftChild._payload > node._payload:
...
if __name__ == '__main__':
    ...
    # print('\n\n\n\n\n\n')
    test_books()
>>>>>>> Fixed rotation and height recalculation, added recalculate and highest child methods
